...
<div class="page-nav">
  {#if currentpage > 1}
    <button
      on:click={() => navigate(parseInt(currentpage) ? 1 : currentpage - 1)}
      ><svg
        xmlns="http://www.w3.org/2000/svg"
        width="24"
        height="24"
        viewBox="0 0 24 24"><path d="M0 12l9-8v6h15v4h-15v6z" /></svg
      ></button
    >
  {/if}

  {#if hasNext}
    <button on:click={() => navigate(parseInt(currentpage) + 1)}
      ><svg
        xmlns="http://www.w3.org/2000/svg"
        width="24"
        height="24"
        viewBox="0 0 24 24"><path d="M24 12l-9-8v6h-15v4h15v6z" /></svg
      ></button
    >
  {/if}
  <!-- {#if currentpage > 1}
